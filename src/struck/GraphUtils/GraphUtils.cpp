...
// Graphing functions
//------------------------------------------------------------------------------
const CvScalar BLACK = Scalar(0, 0, 0);
const CvScalar WHITE = Scalar(255, 255, 255);
const CvScalar GREY = Scalar(150, 150, 150);
...
CvScalar getGraphColor(void) {
    ...
    switch (countGraph) {
        case 1:
            return Scalar(60, 60, 255); // light-blue

        case 2:
            return Scalar(60, 255, 60); // light-green

        case 3:
            return Scalar(255, 60, 40); // light-red

        case 4:
            return Scalar(0, 210, 210); // blue-green

        case 5:
            return Scalar(180, 210, 0); // red-green

        case 6:
            return Scalar(210, 0, 180); // red-blue

        case 7:
            return Scalar(0, 0, 185);   // dark-blue

        case 8:
            return Scalar(0, 185, 0);   // dark-green

        case 9:
            return Scalar(185, 0, 0);   // dark-red
...
IplImage* drawFloatGraph(const float* arraySrc, int nArrayLength, IplImage* imageDst, float minV, float maxV, int width, int height, char* graphLabel, bool showScale) {
    ...
    if (graphLabel != NULL && strlen(graphLabel) > 0) {
        ...
        cvPutText(imageGraph, graphLabel, cvPoint(30, 10), &font, Scalar(0, 0, 0)); // black text
    }
